...
def get_mesh_box(
    ...
    if tracer_type == "abc":
        ...
        if rank == 0:
            logging.info(
                f"Total number of galaxies for tracer_c: {N_gal_c}, mean number density NZ_c: {NZ_c}"
            )
    ...
    if rank == 0:
        if correlation_mode == "auto":
            ...
            mesh_attrs.update(
                {"NZ_a": NZ_a, "NZ_b": NZ_b, "N_gal_a": N_gal_a, "N_gal_b": N_gal_b}
            )
        if tracer_type == "abc":
    ...
    # Mesh generation
    rfield_a = pm_painter(
        data_a["Position"], data_a["WEIGHT"], Nmesh, boxsize, sampler, interlaced, comm
    )
    if rank == 0:
...
def pm_painter(
    Position, WEIGHT, Nmesh, boxsize, sampler, interlaced, comm, boxcenter=None
):
    """
    ...
    pm = ParticleMesh(
        BoxSize=boxsize, Nmesh=Nmesh, dtype="complex128", resampler=sampler, comm=comm
    )
    ...
    if not interlaced:
        ...
        if rank == 0:
            logging.info(
                f"Interlaced painting and R2C done in Rank {rank}, start combining fields"
        )
        ...
        for k, s1, s2 in zip(c1.slabs.x, c1.slabs, c2.slabs):
            kH = sum(
                k[i] * H[i] for i in range(3)
            )  # compute k·H (wavevector dot cell size)
...
def get_mesh_pk_survey(
    ...
    particle_sums_a = compute_particle_sums_pk(
        data_a, randoms_a, correlation_mode, comm
    )

    if rank == 0:
        (
            weight_sum_data_a,
            weight_sum_randoms_a,
            weight_sum_data_wofkp_a,
            weight_sum_randoms_wofkp_a,
            I_sum_data_a,
            I_sum_randoms_a,
            N_sum_data_a,
            N_sum_randoms_a,
    ...
    weight_sum_randoms_a = comm.bcast(
        weight_sum_randoms_a if rank == 0 else None, root=0
    )
    ...
    rfield_data_a = pm_painter(
        data_a["Position"],
        data_a["WEIGHT"] * data_a["WEIGHT_FKP"],
        Nmesh,
        boxsize,
        sampler,
        interlaced,
    ...
    rfield_randoms_a = pm_painter(
        randoms_a["Position"],
        randoms_a["WEIGHT"] * randoms_a["WEIGHT_FKP"],
        Nmesh,
        boxsize,
        sampler,
        interlaced,
    ...
    if correlation_mode == "auto":
        ...
        particle_sums_b = compute_particle_sums_pk(
            data_b, randoms_b, correlation_mode, comm
        )
        if rank == 0:
            (
                weight_sum_data_b,
                weight_sum_randoms_b,
                weight_sum_data_wofkp_b,
                weight_sum_randoms_wofkp_b,
                I_sum_data_b,
                I_sum_randoms_b,
                N_sum_data_b,
                N_sum_randoms_b,
        ...
        rfield_data_b = pm_painter(
            data_b["Position"],
            data_b["WEIGHT"] * data_b["WEIGHT_FKP"],
            Nmesh,
            boxsize,
            sampler,
            interlaced,
        ...
        rfield_randoms_b = pm_painter(
            randoms_b["Position"],
            randoms_b["WEIGHT"] * randoms_b["WEIGHT_FKP"],
            Nmesh,
            boxsize,
            sampler,
            interlaced,
    ...
    if normalization_scheme == "particle":
        if correlation_mode == "auto":
            ...
            raise ValueError(
                "Particle-normalization scheme will not work for cross-correlation!"
            )
        ...
        if correlation_mode == "auto":
            I_mesh_sub = (
                alpha_a
                * np.sum(rfield_data_a * rfield_randoms_a)
                * np.prod(boxsize)
                / np.prod(nmesh)
            )
...
def get_mesh_bk_survey(
    ...
    ), "get_mesh_bk_survey only supports survey-like geometry."
    assert correlation_mode in [
        "auto",
        "cross",
    ], "correlation_mode must be 'auto' or 'cross'."
    ...
    def _prepare_tracer(
        ...
        if rank == 0:
            (
                weight_sum_data,
                weight_sum_randoms,
                weight_sum_data_wofkp,
                weight_sum_randoms_wofkp,
                I_33,
                S_LM_data,
                S_LM_randoms,
            ) = sums_bk
        ...
        if rank == 0:
            logging.info(
                f"Start generating mesh for randoms catalog of tracer_{label}."
            )
    ...
    if normalization_scheme == "particle":
        ...
        ), "Particle-normalization scheme is only applicable for auto-correlation."
        if rank == 0:
            mesh_attrs["I_norm"] = mesh_attrs[f"I_33_a"]
    elif normalization_scheme == "mesh":
        assert (
            correlation_mode == "auto"
        ), "Mesh-normalization scheme is only applicable for auto-correlation."
        rfield_randoms_a = tracer_cache["a"]["rfield_randoms"]
        if rfield_randoms_a is None:
            raise RuntimeError(
                "rfield_randoms_a is required for mesh normalization but is None."
            )
        ...
        if correlation_mode == "auto":
            ...
            if rfield_data_a is None:
                raise RuntimeError(
                    "rfield_data_a is required for mixed-mesh auto normalization but is None."
                )
            ...
            if rank == 0:
                logging.info(
                    f"Start generating meshes for data and randoms catalogs of tracer_a for mixed-mesh normalization."
                )
            ...
            if alpha_scheme == "pypower":
                ...
                sub_weight_sum_randoms_a_odd = np.sum(
                    randoms_a_odd["WEIGHT"] * randoms_a_odd["WEIGHT_FKP"]
                )
                sub_weight_sum_randoms_a_even = np.sum(
                    randoms_a_even["WEIGHT"] * randoms_a_even["WEIGHT_FKP"]
                )
            elif alpha_scheme == "nbodykit":
            ...
            weight_sum_randoms_a_odd = comm.reduce(
                sub_weight_sum_randoms_a_odd, op=MPI.SUM, root=0
            )
            weight_sum_randoms_a_even = comm.reduce(
                sub_weight_sum_randoms_a_even, op=MPI.SUM, root=0
            )
            alpha_a_odd = (
                weight_sum_data_a / weight_sum_randoms_a_odd if rank == 0 else None
            )
            alpha_a_even = (
                weight_sum_data_a / weight_sum_randoms_a_even if rank == 0 else None
            )
            alpha_a_odd = comm.bcast(alpha_a_odd, root=0)
            alpha_a_even = comm.bcast(alpha_a_even, root=0)

            rfield_randoms_a_odd = pm_painter(
                randoms_a_odd["Position"],
                randoms_a_odd["WEIGHT"] * randoms_a_odd["WEIGHT_FKP"],
                Nmesh,
                boxsize,
                sampler,
            ...
            rfield_randoms_a_even = pm_painter(
                randoms_a_even["Position"],
                randoms_a_even["WEIGHT"] * randoms_a_even["WEIGHT_FKP"],
                Nmesh,
                boxsize,
                sampler,
            ...
            del (
                ...
                randoms_a_odd,
                randoms_a_even,
                rfield_randoms_a_odd,
                rfield_randoms_a_even,
            )
            gc.collect()
        else:
            if tracer_type == "abc":
                ...
                if (
                    (rfield_randoms_a is None)
                    or (rfield_randoms_b is None)
                    or (rfield_randoms_c is None)
                ):
                    raise RuntimeError(
                        "random meshes are required for tracer_type='abc' mixed-mesh normalization."
                    )
                sub_I_mesh = vol_per_cell * (
                ...
                rfield_randoms_b = tracer_cache["b"]["rfield_randoms"]
                if (
                    (rfield_data_a is None)
                    or (rfield_randoms_a is None)
                    or (rfield_randoms_b is None)
                ):
                    raise RuntimeError(
                ...
                if (
                    (rfield_randoms_a is None)
                    or (rfield_randoms_b is None)
                    or (rfield_data_b is None)
                ):
                    raise RuntimeError(
                        "R_a, R_b and D_b meshes are required for tracer_type='abb'."
                    )
                sub_I_mesh = vol_per_cell * (
...
def get_N_field(
    ...
    assert (
        geometry == "survey-like"
    ), "get_N_field currently supports survey-like geometry only."
    assert tracer_flag in [
        "a",
        "b",
        "c",
    ], "tracer_flag must be one of 'a', 'b', or 'c'."
    ...
    if rank == 0:
        logging.info(
            f"Start generating N_field for tracer_{tracer_flag} with alpha={alpha:.8e}."
        )

    randoms = catalog_reader(
        catalogs[randoms_key],
        geometry,
        column_names,
        z_range,
        comp_weight_plan,
    ...
    if rank == 0:
        logging.info(
            f"Start generating squared-weight mesh for data catalog of tracer_{tracer_flag}."
        )
    rfield_data = pm_painter(
        data["Position"],
        weight_data_sq,
    ...
    if rank == 0:
        logging.info(
            f"Start generating squared-weight mesh for randoms catalog of tracer_{tracer_flag}."
        )
    rfield_randoms = pm_painter(
        randoms["Position"],
        weight_randoms_sq,
...
def compute_particle_sums_pk(data, randoms, correlation_mode, comm):
    ...
    sub_I_sum_data = np.sum(
        data["WEIGHT"] * data["NZ"] * data["WEIGHT_FKP"] * data["WEIGHT_FKP"]
    )
    sub_I_sum_randoms = np.sum(
        randoms["WEIGHT"]
        * randoms["NZ"]
        * randoms["WEIGHT_FKP"]
        * randoms["WEIGHT_FKP"]
    ...
    gathered_results = comm.gather(
        [
            sub_weight_sum_data,
            sub_weight_sum_randoms,
            sub_weight_sum_data_wofkp,
            sub_weight_sum_randoms_wofkp,
            sub_I_sum_data,
            sub_I_sum_randoms,
            sub_N_sum_data,
            sub_N_sum_randoms,
...
def compute_particle_sums_bk(data, randoms, correlation_mode, angu_config, comm):
    ...
    sub_I_33 = np.sum(
        np.array(randoms["NZ"]) ** 2
        * np.array(randoms["WEIGHT"]) ** 2
    ...
    if correlation_mode == "auto":
        ...
        for mm in range(0, angu_config[2] + 1):
            ...
            if rank == 0:
                logging.info(
                    f"Start calculating S_LM with spherical harmonics Y_{angu_config[2]}^{mm} = {ylm.expr}"
                )
            S_LM_data_mm = np.sum(
                ((data["WEIGHT"] * data["WEIGHT_FKP"])) ** 3
                * np.conj(ylm(data_unit[:, 0], data_unit[:, 1], data_unit[:, 2]))
            )
            S_LM_randoms_mm = np.sum(
                ((randoms["WEIGHT"] * randoms["WEIGHT_FKP"])) ** 3
                * np.conj(
                    ylm(randoms_unit[:, 0], randoms_unit[:, 1], randoms_unit[:, 2])
                )
            )
