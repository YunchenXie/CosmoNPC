...
def h5_reader(comm, files, column_names):
    ...
    for f in [files] if isinstance(files, str) else files:
        ...
        with h5py.File(f, "r") as h5f:
            ...
            if rank == 0:
                logging.info(
                    f"File: {f}, Total Rows: {nrows}, All Columns: {list(h5f.keys())}"
                )
            ...
            if start < nrows:
                ...
                result = (
                    np.concatenate([result, local_data])
                    if len(result) > 0
                    else local_data
                )
...
def npy_reader(data_path, comm):
    ...
    logging.info(
        f"Reading .npy data from disk, Rank {rank}: Rows {start_row} to {end_row-1}, shape {local_data.shape}"
    )
...
def fits_reader(comm, files, column_names):
    ...
    for f in [files] if isinstance(files, str) else files:
        ...
        with fitsio.FITS(f) as fits:
            ...
            if rank == 0:
                logging.info(
                    f"File: {f}, Total Rows: {nrows}, All Columns: {hdu.get_colnames()}"
                )
...
def add_completeness_weight(dr, comp_weight_plan, catalog_type, comm):
    ...
    assert catalog_type in [
        "data",
        "randoms",
    ], "catalog_type must be 'data' or 'randoms'"
    ...
    if "WEIGHT" in dr.dtype.names:
        ...
        dr = np.lib.recfunctions.append_fields(
            dr, "WEIGHT", data=np.zeros(len(dr)), usemask=False
        )
        if alias == None and scheme is None:
            dr["WEIGHT"] = 1.0
            if rank == 0:
                logging.info(
                    "No completeness weight scheme or alias provided. Setting completeness weight to 1.0"
                )
            ...
            if scheme == "boss":
                dr["WEIGHT"] = (
                    dr["WEIGHT_SYSTOT"] * (dr["WEIGHT_NOZ"] + dr["WEIGHT_CP"] - 1.0)
                    if catalog_type == "data"
                    else 1.0
                )
                if rank == 0:
                    logging.info(
                        f"Using BOSS-like completeness weight for {catalog_type}"
                    )
            elif scheme == "eboss":
                dr["WEIGHT"] = (
                    dr["WEIGHT_SYSTOT"] * dr["WEIGHT_NOZ"] * dr["WEIGHT_CP"]
                    if catalog_type == "data"
                    else 1.0
                )
                if rank == 0:
                    logging.info(
                        f"Using eBOSS-like completeness weight for {catalog_type}"
                    )
            elif scheme == "desi":
                raise ValueError("DESI completeness weight scheme not yet implemented.")
        else:
            raise ValueError(
                "Cannot specify both a scheme and an alias for completeness weight."
            )
...
def catalog_reader(
    ...
    if isinstance(data_path, list):
        data_ext = (
            data_path[0].split(".")[-1].lower() if data_path and data_path[0] else None
        )
    else:
    ...
    if geometry == "box-like":
        ...
        if data_ext == "npy":
            ...
            assert (
                column_names is not None and len(column_names) >= 3
            ...
            data_cat = np.zeros(
                len(data_arr), dtype=[("Position", "f8", (3,)), ("WEIGHT", "f8")]
            )
            data_cat["Position"] = np.column_stack(
                [data_arr[:, idx] for idx in position_indices]
            )
            ...
            if "w" in column_names:
                ...
                if rank == 0:
                    logging.info(
                        f"Using {column_names[column_names.index('w')]} as the WEIGHT column"
                    )
            else:
                data_cat["WEIGHT"] = 1.0
                if rank == 0:
            ...
            for axes in [("X", "Y", "Z"), ("x", "y", "z")]:
                if all(axis in data_arr.dtype.names for axis in axes):
                    data_cat["Position"] = np.column_stack(
                        [data_arr[axis] for axis in axes]
                    )
                    if rank == 0:
                ...
                for vel_axes in [("VX", "VY", "VZ"), ("vx", "vy", "vz")]:
                    if all(axis in data_arr.dtype.names for axis in vel_axes):
                        velocities = np.column_stack(
                            [data_arr[axis] for axis in vel_axes]
                        )
                        if rank == 0:
                            logging.info(
                                f"Using {vel_axes} as the velocity columns for RSD"
                            )
                        break
                data_cat["Position"] = add_rsd(
                    comm,
                    data_cat["Position"],
                    velocities,
                    para_cosmo["h"],
            ...
            if "w" in data_arr.dtype.names:
                ...
                if rank == 0:
                    logging.info(
                        "WEIGHT column does not exist in the data. Setting WEIGHT to 1.0"
                    )
        ...
        if data_ext == "npy":
            raise NotImplementedError(
                "Survey-like geometry with .npy file is not yet implemented."
            )
            ...
            if rank == 0:
                logging.info(
                    f"Total objects after applying z_range cut: {total_num_objects}"
                )

            # add completeness weight
            data_arr = add_completeness_weight(
                data_arr, comp_weight_plan, catalog_type, comm
            )
            ...
            if "NZ" in data_arr.dtype.names:
                ...
                if normalization_scheme == "particle":
                    raise ValueError(
                        "NZ column is missing in the catalog, which is required for particle normalization."
                    )
                else:
                    data_cat["NZ"] = 1.0
                    if rank == 0:
                        logging.info(
                            "NZ column does not exist in the catalog, Setting NZ to 1.0"
                        )
...
def ra_dec_z_to_xyz(data_arr, para_cosmo=None, comm=None):
    ...
    if para_cosmo is not None and "h" in para_cosmo and "Omega0" in para_cosmo:
        ...
        # )
        my_cosmo = FlatLambdaCDM(
            H0=100 * para_cosmo["h"] * u.km / u.s / u.Mpc, Om0=para_cosmo["Omega0"]
        )
        if rank == 0:
